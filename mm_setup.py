...
# opentrons_simulate.exe mm_setup.py
# opentrons_execute mm_setup.py
# scp -i Ot2sshKey C:\OpenTronesProtocols\Jordan\mm_setup.py root@169.254.168.129:
# J@$0nRule$
...
#First Code Block Loads all labware.
    ...
    tipsL = protocol.load_labware('opentrons_96_tiprack_300ul', 2)
...
#Asking user if they would like to do custom calibration.
        ...
        if nowCalibrating == "y":
            print("\n\nStarting Calibration: Distribute Plate")
...
#See STAGE 8 for detailed explanation of use.
        ...
        if ((sum(repeatsList)+primerCouples) > 12):
            ...
            errorMessage = input("User,quit with [CTRL+C] and try again.")
...
#Ex. ['A1', 'A1', 'A2', 'A2', 'B1', 'B1', 'B2', 'B2', 'C1', 'C1', 'C2', 'C2'] , ['A1', 'A1', 'A1', 'A1', 'A1', 'A1', 'A2', 'A2', 'A2', 'A2', 'A2', 'A2']
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
